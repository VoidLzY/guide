...
let GodGuide = cc.Class({
    ...
    onLoad() {
        ...
        this.textTh = this.node.getChildByName('text')
        this.textLab = this.textTh.getComponent(cc.Label)
        this.textTh.active = false
        this.stepIndex = 0
        // cc.director.on('drag', (msg) => {
        //     // this._task.step
        //     // console.log(this._task.steps)
        //     if (this._finger) {
        //         this._finger.stopAllActions()
        //     }
        //     this.last_step = null
        //     this.taskZan = this._task.steps
        //     this.arrCheck(this.taskZan, msg.step)
        //     this.taskZan.unshift(this.last_step)

        //     async.eachSeries(this.taskZan, (step, cb) => {
        ...
        // }, this)
        if (this._task) {
            this._task.autorun = true
        }
        this.rectDrag = null
        this.isContuinDrag = false
        this.isSet = false
    },
    ...
    arrCheck(arr, num) {
        ...
        // console.log(num)
        if (arr.length != 0) {
            if (arr[0].command.cmd == 'drag') {
                // console.log(arr[0])
                if (arr[0].command.dragNum == num)
                    return arr
                else {
                    this.last_step = arr[0]
                    arr.splice(0, 1)
                    this.arrCheck(arr, num)
                }
            }
            else {
                this.last_step = arr[0]
    ...
    init() {
        ...
        this.node.on(cc.Node.EventType.TOUCH_START, (event) => {
            ...
            // if (!this.isContuinDrag) {
            //目标区域存在，击中放行
            // console.log(this._targetNode)
            let rect = this._targetNode.getBoundingBoxToWorld();

            if (this.isContuinDrag && !this.isSet) {
                this.isSet = true
                this.rectDrag = rect
            }
            if (this.isContuinDrag) {
                if (this.rectDrag.contains(event.getLocation())) {
                    this.textTh.active = false
                    this.node._touchListener.setSwallowTouches(false);
                    // cc.log('命中目标节点，放行');
                } else {
    ...
    run() {
        ...
        if (this._task.steps && this.stepIndex < (this._task.steps.length)) {
            this._mask.node.active = this._task.mask || true;
            this.scheduleOnce(() => {
                this._processStepCommand(this._task.steps[this.stepIndex], () => {
                    this.stepIndex += 1
                    this.run();
                });
            }, this._task.steps[this.stepIndex].delayTime || 0);
        } else {
            console.log('已完成任务')
            this._task = null;
            // cc.log('任务结束');
    ...
    _processStep(step, callback, areaBack) {
        // console.log(step)
        if (!step) {
            this.taskZan = this.taskZan.slice(2, this.taskZan.length)
            // console.log(this.taskZan)
            async.eachSeries(this.taskZan, (step, cb) => {
                this._processStep(step, cb);
    ...
    fingerToNode(node, text, pos, cb) {
        ...
        let callFunc = cc.callFunc(() => {
            if (text) {
                // console.log(text)
                this.showtextSec(text, pos)
                // console.log(this.textTh)
    ...
    _processStepCommand(step, cb) {
        ...
        if (cmd == godCommand.drag) {
            this.isContuinDrag = true
        }
        if (cmd) {
    ...
    findSecond(value, text, pos, cb) {
        ...
        locator.locateNode(root, value, (error, node) => {
            ...
            // cc.log('定位节点成功');
            let rect = this._focusToNodeSecond(node);
            if (cb) {
                cb(node, rect);
            }
        });
    ...
    //显示文本
    showText(text, posY, callback) {
        // this._text.once('click', callback);
        // console.log(this._text.position)
        // console.log(posY)
        // console.log(text)
        if (posY != 'null') {
            this._text.getChildByName('label').y = posY
        } else {
            this._text.getChildByName('label').y = 0
        }
        // console.log(this._text.position)
    ...
    showtextSec(text, posY) {
        this.textTh.active = true
        // console.log(this.textTh)
        // this._text.active=true
        if (posY != 'null' && posY != NaN) {
            this.textTh.y = posY
        } else {
            this.textTh.y = 0
        }
